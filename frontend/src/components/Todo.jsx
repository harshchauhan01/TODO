...
import { MdDelete, MdChevronLeft, MdChevronRight, MdCloud, MdCloudOff, MdSync } from "react-icons/md";
import toast from "react-hot-toast";
import { useOfflineStatus, useOfflineSync } from "../hooks/useOffline";
import { addToSyncQueue, getAllTasks, saveTask } from "../utils/db";
...
const Todo = () => {
  ...
  const fetchUser = async () => {
    ...
    try {
      ...
      setUser(res.data);
      localStorage.setItem("todo_user", JSON.stringify(res.data));
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (isOnline) {
        ...
        const offlineTasks = await getAllTasks(user?.id);
        setData(Array.isArray(offlineTasks) ? offlineTasks : []);
      }
    } catch (fetchError) {
      if (!isOnline) {
        try {
          ...
          setError("Unable to load tasks. Please reconnect once to refresh local cache.");
        }
      } else {
  ...
  const handleDelete = async (id) => {
    try {
      if (!isOnline) {
        ...
        toast.success("Task deleted (queued for sync)");
        return;
      }
  ...
  const handleTaskCompleted = async (id, completed) => {
    try {
      if (!isOnline) {
        setData((prev) => prev.map((task) => (task.id === id ? { ...task, completed: !completed } : task)));
        await addToSyncQueue("update", { id, completed: !completed });
        toast.success(completed ? "Marked pending (queued)" : "Marked completed (queued)");
        return;
      }

      await api.post(`/api/todo/${id}/mark_complete/`);
      await fetchTasks();
      toast.success(completed ? "Marked pending" : "Marked completed");
    } catch {
  ...
  return (
    <main className="page-wrap min-h-screen pb-14 pt-4 sm:pt-6">
      <div className="mb-4 flex items-center justify-between gap-2 px-6 sm:px-0">
        <div
        ...
        >
          {isOnline ? (
            <>
              <MdCloud size={18} /> Online
            </>
          ) : (
            <>
              <MdCloudOff size={18} /> Offline Mode
            </>
          )}
        </div>

        <button
          onClick={offlineSync.manualSync}
          disabled={!isOnline || offlineSync.syncing}
          className="flex items-center gap-2 rounded-lg bg-blue-50 px-3 py-2 text-sm font-medium text-blue-700 transition hover:bg-blue-100 disabled:opacity-50"
        >
          <MdSync size={18} className={offlineSync.syncing ? "animate-spin" : ""} />
      ...
      <div className="space-y-6">
        ...
        {showForm && (
          <section className="rise-in">
            <CustomForm onSuccess={handleTaskCreated} onCancel={() => setShowForm(false)} user={user} />
          </section>
        )}
